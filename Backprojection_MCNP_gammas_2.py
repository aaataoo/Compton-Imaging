...
def run_BP(Doubles_File_Name, Radius, scipy=None):
    ...
    def Apply_E_Cuts(Doubles_Data): #filtered the double events in our ROI, 做能量切片?
        Data_Out =  np.zeros_like(Doubles_Data)          #??????????check
        Edep_1_Counter = 0
        Edep_2_Counter = 0
        Edep_Both = 0
    ...
    def Convert_Channel_to_Position(E_Cut_Applied_Data):
        Data_Out = np.zeros_like(E_Cut_Applied_Data) #???????????????????check
        good_counter = 0
        in_bar_Counter = 0    
    ...
    def Creating_Real_Projections(Event, Radius, Theta_Rad, Phi_Rad): 
        ...
        Alpha = float((1 + 0.511 * (1/Event[11] - 1/Event[10]))**2) # (1+MeC^2*(1/Etotal - 1/E_prime))
        print(Alpha)
