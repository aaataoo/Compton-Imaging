...
@author: artao
the input file should be .inp (binary) file
"""
...
total_nf_double_counts = 0
total_double_counts = 0
...
@njit(parallel=True)
def process_all_double_events(Coincident_Events_nf, data_g, data_raw, Bar_Num_to_MCNP_Cell, total_double_counts):
    n = len(Coincident_Events_nf)
    ...
    for i in prange(n):
        t0 = Coincident_Events_nf[i][0]
        t1 = Coincident_Events_nf[i][1]
        ...
        # find out the history id
        lookup_o = data_g[idx_B1, 0]
        recorded_history[i] = lookup_o

        # find out the index in the raw data
        matched_idx = []
...
#--------------------------------------------------------------2
if multiple_files:
    Coincident_Data_Out = np.zeros((100000*nFiles, 12)) 
    for j in range(nFiles):
        ...
        Coincident_Events_nf = Finding_Coincident_Events(Sorted_All_Start_Times, Min_Time_Window, Max_Time_Window)
        Coincident_Events_nf = [list(data) for data in Coincident_Events_nf]
        
        
        
        recorded_history = [] # in the later data verification, this term is easy to lookup in raw data
        
        Coincident_Events_nf = np.array(Coincident_Events_nf)
        Bar_Num_to_MCNP_Cell = Bar_Num_to_MCNP_Cell.astype(np.int64)
        
        
        Coincident_Data_Out, recorded_history = process_all_double_events(
            Coincident_Events_nf,
    ...
    filtered_double_counts = len(Coincident_Data_Out)
    ...
    print("Number of Double Events: "+str(filtered_double_counts))
    print("\n")
    total_double_counts = total_double_counts + filtered_double_counts
    total_nf_double_counts = total_nf_double_counts + len(Coincident_Events_nf)
    
print('Total double scatter events found : '+str(total_nf_double_counts))
print('Total double scatter events written to file (After discriminate): '+str(total_double_counts))
print("\n")
...
Writing_Data(Coincident_Data_Out[:total_double_counts])
Out.close()


#%%
#not a necessary part but easier to debug
...
file_destination = "output_data.xlsx"
Writing_Data_to_xlsx(Coincident_Data_Out[:total_double_counts], os.path.join(folder, file_destination))
